...
, where $P$ is the Phytoplankton biomass, $\overline{P_h}$ the average photosynthisis in euphotic zone, and $G$ the grazing rate.
...
$$
\overline{P_h}  = \frac{\textcolor{saddlebrown}{p}~\textcolor{steelblue}{I_0}}{\textcolor{steelblue}{k~z_\text{euph}}}~(1-\mathrm{e}^{-\textcolor{steelblue}{k~z_\text{euph}}})~\textcolor{steelblue}{L_\text{N}~L_\text{V}}
$$

Parameters are shown in [red]{style="color:saddlebrown"} and forcing variables in [blue]{style="color:steelblue"}

**Parameters:**

- $\textcolor{saddlebrown}{p}$ = 2.5 $\mathrm{\frac{gC~gC^{-1}~d^{-1}}{g~cal~cm^{-2}~min^{-1}}}$ *Photosynthetic rate per unit irradiance*


**Forcing variables:**

- $\textcolor{steelblue}{I_0}$: *Light at seasurface* in $\mathrm{\mu E~m^{-2}~s^{-1}}$
- $\textcolor{steelblue}{k}$: *Light attenuation in the euphotic zone*$\mathrm{m^{-1}}$
- $\textcolor{steelblue}{L_\text{N}}$: *Limiting effects of turbulence*, unitless
- $\textcolor{steelblue}{L_\text{V}}$: *Limiting effects of nutrients*, unitless
- $\textcolor{steelblue}{z_\text{euph}}$: *Depth of the euphotic zone* in $\mathrm{m}$

$\textcolor{steelblue}{L_\text{V}}$ is calculated as

$$
\textcolor{steelblue}{L_\text{V}} = \min(1, \frac{\textcolor{steelblue}{z_\text{euph}}}{\textcolor{steelblue}{z_\text{mix}}})
$$
, where $\textcolor{steelblue}{z_\text{mix}}$ is the depth of the mixed layer in $\mathrm{m}$


$\textcolor{steelblue}{z_\text{euph}}$ is calculated as

$$
\textcolor{steelblue}{z_\text{euph}} = \log(\frac{\textcolor{steelblue}{I_0}}{\textcolor{saddlebrown}{I_\text{min}}})
$$
, where $\textcolor{saddlebrown}{I_\text{min}}$ is a parameter defining the light intensity at the base of the euphotic zone: 0.0015 $\mathrm{g~cal~cm^{−2}~min^{−1}}$
...
$$
R = \textcolor{saddlebrown}{R_0}~\mathrm{e}^{\textcolor{saddlebrown}{r}~\textcolor{steelblue}{\text{temp}}}
$$

**Parameters:**

- $\textcolor{saddlebrown}{R_0}$: 0.0175 $\text{d}^{-1}$ *Respiration rate of phytoplankton at 0 ◦C*, in $\mathrm{d^{-1}}$
- $\textcolor{saddlebrown}{r}$: 0.069 $\text{°C}^{-1}$ *Temperature dependency of respiration rate*

$\textcolor{saddlebrown}{r}$ is equal $Q_{10} = 2$, i.e. doubling of rate for 10 °C increase in temperature: $\mathrm{e}^{10 \text{°C}~0.069~\textrm{°C}^{-1}}\approx2$

**Forcing Variables:**

- $\textcolor{steelblue}{\text{temp}}$: *Surface (?) seawater temperature* in $\textrm{°C}$
...
$$
G = \textcolor{saddlebrown}{g}~\textcolor{steelblue}{zoo_{\text{bm}}}
$$


**Parameters:**

- $\textcolor{saddlebrown}{g}$: 0.0075 $\mathrm{\frac{d^{-1}}{gC~m^{-2}}}$ *Clearance rate of grazers*


**Forcing Variables:**

- $\textcolor{steelblue}{zoo_{\text{bm}}}$: *Biomass of zooplankton* in $\mathrm{gC~m^{-2}}$ (?)
...
Lower value explored is 0.5 \* baseline parameter value and upper value 2 \* baseline value.
...
In @fig-global_sensitivity_lhs and @fig-global_sensitivity_norm, the timing is mainly in 3 days, independed on the other values, which correlate to the three blooms that seem to depend only on the forcing variables.