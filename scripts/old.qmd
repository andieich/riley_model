...
# to improe fitting, add values from day 0 as day 365
dat_forc <- dat_forc %>%
  # for some reason, forcing data is 0 base, i.e. 1.1. corresponds to day 0
  mutate(days = days + 1) # %>%
#  bind_rows(c("days" = 365, dat_forc[1, 2:7]))
...
```{r}
#| include: false
plot_forcings_gam <- function(gam_list, dat_forc, days = 1:365) {
  plots <- lapply(names(gam_list), function(v) {
    # Predict smooth values for all days
    pred_df <- data.frame(
      days = days,
      value = predict(gam_list[[v]], newdata = data.frame(days = days))
    )

    # Build plot for this variable
    ggplot(pred_df, aes(x = days, y = exp(value))) +
  ...
  # name plots
  names(plots) <- names(gam_list)

  return(wrap_plots(plots))
}

plot_forcings_gam(gam_list, dat_forc)
```
...
m_riley <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    I_0_t <- predict(gam_list.I_0, newdata = data.frame(days = time)) %>% exp()
    k_t <- predict(gam_list.k, newdata = data.frame(days = time)) %>% exp()
    L_N_t <- predict(gam_list.L_N, newdata = data.frame(days = time)) %>% exp()
...
data.frame(
  days = 1:365,
  I_0 = predict(gam_list$I_0, newdata = data.frame(days = 1:365))
) %>%
...
data.frame(
  days = 1:365,
  Temp = predict(gam_list$Temp, newdata = data.frame(days = 1:365))
) %>%
...
data.frame(
  days = 1:365,
  k = predict(gam_list$k, newdata = data.frame(days = 1:365))
) %>%
  ggplot(aes(x = days, y = k)) +
  geom_line() +
  geom_point(data = dat_forc, aes(y = k)) +
  theme_light()
```